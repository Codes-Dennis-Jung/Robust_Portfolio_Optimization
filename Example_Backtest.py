...
for strategy_name, results in [
    ...
    plt.savefig(f"{strategy_name.lower().replace(' ', '_')}_dashboard_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png",
                dpi=300, bbox_inches='tight')
    plt.close(fig)
    
    # Save detailed results
    output_prefix = f"{strategy_name.lower().replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
